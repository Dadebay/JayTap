...
class HomeService {
  ...
  Future<List<BannerModel>> fetchBanners() async {
    try {
      final response = await _apiService.getRequest(ApiConstants.banners,
          requiresToken: false);
  ...
  Future<List<CategoryModel>> fetchCategories() async {
    ...
    if (response != null && response is List) {
      List<CategoryModel> categories = response
  ...
  Future<List<RealtorModel>> fetchRealtors() async {
    try {
      final response = await _apiService.getRequest(ApiConstants.realtors,
          requiresToken: false);

      if (response != null && response['results'] is List) {
        final realtorResponse = RealtorResponse.fromJson(response);
        return realtorResponse.results
            .where((user) => user.blok == false)
            .toList();
      } else {
        return [];
      }
